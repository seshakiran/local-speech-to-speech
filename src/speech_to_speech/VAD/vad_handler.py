...
class VADHandler(BaseHandler[VADIn, VADOut]):
    ...
    def setup(
        ...
        if os.environ.get("HF_HUB_OFFLINE") == "1":
            cached_repo = os.path.join(torch.hub.get_dir(), "snakers4_silero-vad_master")
            if not os.path.isdir(cached_repo):
                raise RuntimeError(
                    f"Offline Silero VAD cache is missing: {cached_repo}. "
                    "Start once online before enabling offline mode."
                )
            self.model, _ = torch.hub.load(cached_repo, "silero_vad", source="local")
        else:
