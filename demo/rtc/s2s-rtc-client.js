...
export class S2sRtcRealtimeClient extends EventTarget {
  ...
  constructor(options) {
    ...
    /** @type {HTMLAudioElement | null} Native WebRTC playback sink. Keeping
     * remote audio on this path gives browser AEC its far-end reference. */
    this._quirkAudio = null;
    /** @type {GainNode | null} Silent sink that keeps the analyser graph live
     * without playing a second copy through WebAudio. */
    this._analysisSink = null;
  ...
  // Output: native <audio> playback (so WebRTC AEC sees the far-end reference),
  //         plus a silent remoteSrc ─→ outAnalyser branch for the orb.
  ...
  _onRemoteTrack(e) {
    ...
    // Play through the native WebRTC media path. Routing audible output through
    // AudioContext can deprive the browser echo canceller of its clean far-end
    // reference and make the assistant transcribe itself.
    const quirk = new Audio();
    quirk.autoplay = true;
    quirk.playsInline = true;
    quirk.srcObject = stream;
    this._quirkAudio = quirk;
    void quirk.play().catch((err) => console.warn("[rtc] remote audio play failed:", err));
  ...
  /** Add a text turn to the conversation (used by private voice onboarding).
   * @param {string} text */
  sendUserText(text) {
    if (!text) return;
    this._send({
