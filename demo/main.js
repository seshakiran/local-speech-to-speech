...
import { $, escHtml, truncateError, DEBUG } from "./ui/dom.js";
...
const DEFAULT_INSTRUCTIONS =
  ...
  "Never claim to use a hosted service or cloud model.";
...
const STORAGE_KEYS = {
  ...
  voice: "s2s.ws.voice",
  instructions: "local.voice.instructions.v2",
  tools: "s2s.ws.tools",
  ...
  // v3 makes localhost WebSocket the offline-safe default. A new key avoids
  // carrying forward WebRTC choices saved before interface loss was handled.
  transport: "s2s.transport.v3",
  userName: "local.voice.userName",
};
...
function loadSettings() {
  return {
    ...
    // WebSocket stays on loopback when Wi-Fi is disabled. WebRTC remains an
    // option for users who keep an active LAN interface.
...
function effectiveInstructions() {
  const identity = userName
    ? ` The user's name is ${userName}. Address them naturally by name, but not in every reply.`
    : "";
  const base = settings.instructions + identity;
  return activeToolDefs().length ? base + TOOL_USE_HINT : base;
}

/** Turn a short spoken introduction into a conservative display name. */
function nameFromTranscript(text) {
  const cleaned = text
    .trim()
...
function saveUserName(name) {
  ...
  onboardingPhase = userName ? "complete" : "idle";
  inputUserName.value = userName;
  applyLocalBranding();
}

function applyLocalBranding() {
  const title = /** @type {HTMLAnchorElement} */ (document.querySelector(".ident-title"));
  title.textContent = userName ? `${userName}'s Local Speech-to-Speech` : "Local Speech-to-Speech";
  title.removeAttribute("href");
  title.removeAttribute("target");
  title.removeAttribute("rel");
  const legacyMeta = /** @type {HTMLElement | null} */ (document.querySelector(".ident-meta"));
  if (legacyMeta) legacyMeta.remove();
  profileModel.textContent = localConfig.llmModel;
  ...
  footer.textContent = userName
    ...
    : "Private voice AI · nothing leaves this Mac";

  const intro = /** @type {HTMLElement | null} */ (aboutModal.querySelector(".about-intro"));
  if (intro) {
    intro.innerHTML = `<p>This is your private voice assistant. Speech detection, transcription, reasoning, and voice synthesis run locally on this Mac.</p>`;
  }
  const pipeline = /** @type {HTMLElement | null} */ (aboutModal.querySelector(".about-pipeline"));
  if (pipeline) {
    pipeline.innerHTML = `
      <p class="pipeline-title">Active local setup</p>
      <div class="setup-grid">
        <span>Speech detection</span><strong>Silero VAD</strong>
        <span>Transcription</span><strong>${escHtml(localConfig.sttModel)}</strong>
        <span>Assistant</span><strong>${escHtml(localConfig.llmModel)}</strong>
        <span>Runtime</span><strong>${escHtml(localConfig.llmProvider)}</strong>
        <span>Voice</span><strong>${escHtml(localConfig.ttsModel)}</strong>
        <span>Privacy</span><strong>Localhost · offline-ready</strong>
      </div>`;
  }
  toolWebRow.hidden = true;
  const webKey = /** @type {HTMLElement | null} */ (document.querySelector(".tools-key"));
  if (webKey) webKey.hidden = true;
}
...
function syncTransportUi() {
  ...
  transportHint.textContent = selectable
    ? "WebSocket stays on localhost when Wi-Fi is off. WebRTC needs an active network interface."
...
const MIC_CONSTRAINTS = {
  audio: {
    ...
    // Chromium ignores unsupported constraints; Safari/macOS can use voice
    // isolation when exposed by the selected input device.
    voiceIsolation: true,
    channelCount: 1,
  },
};
...
async function doStart(audioContext = null) {
  ...
  c.addEventListener("status", (e) => {
    ...
    if (detail.status === "connected" && !userName && onboardingPhase === "idle") {
      onboardingPhase = "assistant-asking";
      setCaption("A quick introduction…", "muted");
      c.sendUserText(
        "This is a private onboarding step. Ask me in one short spoken sentence what name you should call me. Do not ask anything else.",
      );
      c.requestResponse();
    }
  });
  c.addEventListener("transcript", (e) => {
    ...
    if (d.role === "user" && !d.partial && onboardingPhase === "awaiting-name") {
      ...
      if (spokenName) {
        saveUserName(spokenName);
        c.updateSession({ instructions: effectiveInstructions() });
        setCaption(`Nice to meet you, ${spokenName}`);
      }
    }
  });
